...
@Component({
  selector: 'app-take-cycleeva',
  templateUrl: './take-cycleeva.component.html',
  styleUrls: ['./take-cycleeva.component.scss']
})
export class TakeCycleevaComponent implements OnInit {
  pageTitle;
  ...
  // 获取评估试卷信息
  paperInfo() {
    this.cycleevaService.getCyCleevaPaper(this._eid).subscribe(
  ...
  doSubmit() {
    ...
    this.cycleevaService.submitCycleeva(eid, this._postData).subscribe(
      data => {
        if (data.status === 1) {
